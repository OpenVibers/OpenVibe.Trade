'use strict';
// Staff powers come from the openvibe-contracts staff map (ADR-022): staff.editorial.manage makes a person an editor (TRADE_EDITORS names more).
...
(function walk(dir) {
    for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        fs.readFileSync(f, 'utf8').split('\n').forEach((line, i) => {
            if (/STAFF_ROLES|claims\.role\s*[!=]==?|\.(has|includes)\(\s*(claims|u|user|p)\.role\b|\brole\s*[!=]==?\s*['"](admin|global_mod|moderator)['"]/.test(line)) offenders.push(`${path.relative(path.join(__dirname, '..'), f)}:${i + 1}`);
        });
    }
